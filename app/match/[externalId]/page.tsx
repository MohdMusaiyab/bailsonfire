...
export default async function MatchRoastPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F9F6EF] relative">
      <div className="fixed inset-0 pointer-events-none opacity-10 bg-[url('https://www.transparenttextures.com/patterns/old-paper.png')] mix-blend-multiply" aria-hidden="true" />
      {/* Architectural vertical lines — mirror homepage */}
      <div className="fixed left-[5%] top-0 bottom-0 w-0.5 bg-[#2C2B28] hidden xl:block pointer-events-none" aria-hidden="true" />
      <div className="fixed right-[5%] top-0 bottom-0 w-0.5 bg-[#2C2B28] hidden xl:block pointer-events-none" aria-hidden="true" />
      ...
      <main className="mx-auto max-w-3xl px-6 md:px-12 py-16 relative z-10">
        ...
        {/* Section divider */}
        <div className="border-b-2 border-dashed border-[#2C2B28] mb-12" aria-hidden="true" />
            ...
            <div className="pl-6 border-l-4 border-[#9B2C2C]">
              <div className="prose prose-neutral max-w-none">
                {match.summary.content.split('\n').map((para, i) =>
                  para.trim() ? (
                    <p key={i} className="text-[1.1rem] md:text-[1.25rem] font-serif leading-[1.8] text-[#3A3126] mb-6">
                      {para}
                    </p>
                  ) : null
                )}
              </div>
              <p className="mt-8 pt-4 border-t-2 border-dashed border-[#2C2B28]/20 text-[0.65rem] font-mono font-bold uppercase tracking-widest text-[#6B5E4A]">
                Author: {match.summary.aiModel}
              </p>
            </div>
          ) : (
        ...
        {/* Section divider */}
        <div className="border-b-2 border-dashed border-[#2C2B28] mb-10" aria-hidden="true" />
